...
- [ ] T010 SQLite TranscriptStore in `src/ambient_recorder/storage/transcripts.py`: tables transcripts/transcript_segments/transcription_jobs, seq assignment, current_transcript (newest non-failed), segments_after, list/get, open_jobs, next_queued; unit tests in `tests/unit/test_transcript_store.py` (seq monotonic, current selection, supersede-by-insert)
- [ ] T011 [P] Attribution rule (pure) in `src/ambient_recorder/transcription/attribution.py` + AttributionConfig (6 dB, 60% overlap defaults) in `src/ambient_recorder/config.py`; unit tests in `tests/unit/test_attribution.py`: bleed dropped, genuine overlap kept, symmetric case, thresholds boundary
- [ ] T012 [P] Engine readiness/degradation in `src/ambient_recorder/transcription/readiness.py`: free-VRAM probe (nvidia-smi / ctranslate2 API, tolerant of absence), model-presence check in `data/models/`, policy medium→small-cuda→small-cpu, `model_missing` reason with the exact download command; unit tests with injected probes in `tests/unit/test_readiness_policy.py`
...
- [ ] T022 [US1] **[GATE-C — halt for human approval before this task]** Install `[transcription]` extra, download Whisper `medium` (and `distil-large-v3` for comparison) into `data/models/` via `scripts/fetch_models.py`; measure and record real VRAM (weights, peak during transcribe) + throughput on the RTX 4070 into research.md R2, confirm/adjust the model choice
...
- [ ] T027 [P] [US2] Not-ready-at-start path: readiness false → live transcript created as `failed: engine_not_ready` immediately (SC-005); integration test in `tests/integration/test_engine_not_ready.py`
...
- [ ] T031 [US3] Integration tests in `tests/integration/test_on_demand.py`: legacy session (no transcript) → completed; session with live transcript → new current + old superseded (list shows both, get-by-id works); failed job → retry creates a fresh attempt
...
- [ ] T034 [US4] Priority/yield integration test in `tests/integration/test_transcription_priority.py`: slow fake engine + on-demand job in progress → start new session → start ≤ generous CI ceiling, live chunks processed before remaining on-demand chunks
