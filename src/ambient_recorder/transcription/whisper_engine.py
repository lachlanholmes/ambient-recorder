...
class WhisperEngine:
    def __init__(self, model: str, device: str, compute_type: str, models_dir: Path,
                 language: str = "en"):
    ...
    def transcribe(self, pcm16k_mono: bytes, *, beam_size: int = 1,
        ...
        try:
            ...
            segments, _info = self._model.transcribe(
                audio, beam_size=beam_size, language=self._language,
            ...
            for s in segments:
                ...
                if not text or s.end <= s.start:
                    continue
                out.append(RawSegment(start_s=float(s.start), end_s=float(s.end), text=text,
