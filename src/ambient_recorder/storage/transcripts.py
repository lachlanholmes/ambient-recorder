...
class SqliteTranscriptStore:
    ...
    def delete_segments(self, transcript_id: str) -> int:
        with self._lock, self._db:
    ...
    def set_engine(self, transcript_id: str, engine: str, model: str) -> None:
        with self._lock, self._db:
            self._db.execute(
                "UPDATE transcripts SET engine=?, model=? WHERE id=?",
    ...
    def list_transcripts(self, session_id: str) -> list[TranscriptSummary]:
        ...
        for r in rows:
            t = self._t(r)
            superseded = (
                t.state not in (TranscriptState.FAILED, TranscriptState.PENDING)
    ...
    def get_job(self, transcript_id: str) -> TranscriptionJob | None:
        with self._lock:
            row = self._db.execute(
                "SELECT * FROM transcription_jobs WHERE transcript_id=?", (transcript_id,)
            ).fetchone()
    ...
    def segments_after(self, transcript_id: str, after: int) -> list[TranscriptSegment]:
        with self._lock:
            rows = self._db.execute(
                "SELECT * FROM transcript_segments WHERE transcript_id=? AND seq>? ORDER BY seq",
                (transcript_id, after),
            ).fetchall()
        return [
            TranscriptSegment(
    ...
    def _t(r: sqlite3.Row) -> Transcript:
        return Transcript(
            ...
            mode=TranscriptMode(r["mode"]),
            state=TranscriptState(r["state"]),
            final=bool(r["final"]),
            engine=r["engine"],
            model=r["model"],
    ...
    def _j(r: sqlite3.Row) -> TranscriptionJob:
        return TranscriptionJob(
            ...
            priority=r["priority"],
            progress_chunks=r["progress_chunks"],
...
def reconcile_transcription(store: SqliteTranscriptStore, active_session_id: str | None) -> dict:
    ...
    for job in store.open_jobs():
        if job.mode == TranscriptMode.LIVE:
            ...
            # Restart from scratch (FR-006): drop segments the dead run had
            # already written, or they would be emitted twice (field: a
            # requeued 69-min job produced every segment in duplicate).
            store.delete_segments(job.transcript_id)
            store.update_job(
                job.transcript_id, state=JobState.QUEUED, started_at=None, progress_chunks=0
            )
            counts["requeued"] += 1
